...
export class AddressOperations {
  ...
  static async addAddress(userId, addressData) {
    try {
      ...
      if (addressData.isDefault) {
        await db.collection(collections.addresses).updateMany({ userId }, { $set: { isDefault: false } })
      }
  ...
  static async updateAddress(userId, addressId, updates) {
    try {
      ...
      if (updates.isDefault) {
        await db
          .collection(collections.addresses)
          .updateMany({ userId, id: { $ne: addressId } }, { $set: { isDefault: false } })
      }
  ...
  static async setDefaultAddress(userId, addressId) {
    try {
      ...
      const result = await db
        .collection(collections.addresses)
        .updateOne({ userId, id: addressId }, { $set: { isDefault: true, updatedAt: new Date() } })
...
export class PaymentMethodOperations {
  ...
  static async addPaymentMethod(userId, paymentData) {
    try {
      ...
      if (paymentData.isDefault) {
        await db.collection(collections.paymentMethods).updateMany({ userId }, { $set: { isDefault: false } })
      }
  ...
  static async updatePaymentMethod(userId, paymentMethodId, updates) {
    try {
      ...
      if (updates.isDefault) {
        await db
          .collection(collections.paymentMethods)
          .updateMany({ userId, id: { $ne: paymentMethodId } }, { $set: { isDefault: false } })
      }
  ...
  static async setDefaultPaymentMethod(userId, paymentMethodId) {
    try {
      ...
      const result = await db
        .collection(collections.paymentMethods)
        .updateOne({ userId, id: paymentMethodId }, { $set: { isDefault: true, updatedAt: new Date() } })
...
export class InvoiceOperations {
  ...
  static async getAllInvoices() {
    try {
      ...
      const invoices = await db.collection(collections.invoices).find({}).sort({ createdAt: -1 }).toArray()
