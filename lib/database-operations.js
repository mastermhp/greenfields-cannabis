...
export { connectToDatabase } from "./mongodb.js"
...
export class CategoryOperations {
  static async createCategory(categoryData) {
    try {
      ...
      console.log("Database: Creating category:", category)

      const result = await db.collection(collections.categories).insertOne(category)

      console.log("Database: Category created with ID:", result.insertedId)
  ...
  static async getAllCategories() {
    try {
      ...
      console.log("Database: Fetching all categories")
      const categories = await db.collection(collections.categories).find({}).sort({ createdAt: -1 }).toArray()
      console.log(`Database: Found ${categories.length} categories`)
  ...
  static async getCategoryById(id) {
    try {
      const { db } = await connectToDatabase()
      console.log("Database: Looking for category with ID:", id)
      ...
      // First try with the id field
      category = await db.collection(collections.categories).findOne({ id })
      ...
      console.log("Database: Found category:", category ? "Yes" : "No")
      return category
    } catch (error) {
  ...
  static async updateCategory(id, updates) {
    try {
      const { db } = await connectToDatabase()
      console.log("Database: Updating category with ID:", id)
      ...
      console.log("Database: Category updated successfully")
  ...
  static async deleteCategory(id) {
    try {
      const { db } = await connectToDatabase()
      console.log("Database: Attempting to delete category with ID:", id)
      ...
      console.log("Database: Category found, proceeding with deletion:", category.name)

      // Check if category is being used by products
      const productsUsingCategory = await db.collection(collections.products).countDocuments({
        $or: [{ category: id }, { categoryId: id }, { category: category.slug }],
      })

      if (productsUsingCategory > 0) {
        throw new Error(`Cannot delete category. ${productsUsingCategory} products are using this category.`)
      }
      ...
      console.log("Database: Delete result:", result.deletedCount > 0 ? "Success" : "Failed")
...
export class UserOperations {
  ...
  static async deleteUser(userId) {
    try {
      ...
      const result = await collection.findOneAndDelete({ id: userId })
      return result.value
    } catch (error) {
...
export async function initializeDatabase() {
  try {
    ...
    if (!adminExists) {
      ...
      console.log("Creating admin user with plain text password for testing...")
      ...
      console.log("Admin user created successfully with plain text password")
    } else {
      console.log("Admin user already exists")
      console.log("Current admin password type:", typeof adminExists.password)
      console.log("Current admin password length:", adminExists.password?.length)

      // If the password is hashed and causing issues, reset it to plain text for testing
      if (adminExists.password && adminExists.password.length > 20) {
        console.log("Resetting admin password to plain text for testing...")
        await db
          .collection(collections.users)
          .updateOne({ email: "admin@greenfields.com" }, { $set: { password: "admin123", updatedAt: new Date() } })
        console.log("Admin password reset to plain text")
      }
    }
...
export class UserDocumentOperations {
  static async uploadDocument(documentData) {
    try {
      ...
      console.log("Creating document:", document.fileName)
  ...
  static async getDocumentById(documentId) {
    try {
      ...
      const document = await db.collection(collections.userDocuments).findOne({ id: documentId })
  ...
  static async deleteDocument(documentId) {
    try {
      ...
      const result = await db.collection(collections.userDocuments).deleteOne({ id: documentId })
  ...
  static async getDocumentsByUserId(userId) {
    try {
      ...
      const documents = await collection.find({ userId: userId }).toArray()
      return documents
    } catch (error) {
