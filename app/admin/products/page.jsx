      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="bg-[#111] border-[#333]">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-3xl font-bold text-green-400 mt-2">{products.filter((p) => p.stock > 0).length}</p>
          </CardContent>
        </Card>
        <Card className="bg-[#111] border-[#333]">
          <CardContent className="p-6 text-center">
        ...
        <Card className="bg-[#111] border-[#333]">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-3xl font-bold text-red-400 mt-2">{products.filter((p) => p.stock === 0).length}</p>
          </CardContent>
        </Card>
      </div>
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-beige text-sm capitalize">{product.category}</p>
                  </div>

                  <div className="flex justify-between items-center">
                    <span className="text-[#D4AF37] font-bold text-xl">${product.price.toFixed(2)}</span>
                    <span className="text-beige text-sm">Stock: {product.stock}</span>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="bg-[#222] p-2 rounded">
                      <span className="text-beige">THC: </span>
                      <span className="text-white">{(product.thcContent * 100).toFixed(1)}%</span>
                    </div>
                    <div className="bg-[#222] p-2 rounded">
                      <span className="text-beige">CBD: </span>
                      <span className="text-white">{(product.cbdContent * 100).toFixed(1)}%</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between pt-2">
                    <span className="text-beige text-sm">
                      {product.featured ? "Featured" : "Regular"} • {product.rating} ⭐
                    </span>
                    <DropdownMenu>
